---
title: Flaxon GraphQL & Admin Guide
description: Learn how to build GraphQL APIs and manage data with the Flaxon Admin Dashboard.
sidebar_position: 1
---

# Flaxon GraphQL & Admin Guide

> Learn how to build modern GraphQL APIs and powerful administration panels with Flaxon.

---

# Overview

Flaxon includes two powerful modules that make it easy to build production-ready applications:

- **GraphQL** — Build flexible APIs with queries, mutations, and subscriptions.
- **Admin Dashboard** — Manage application data through an auto-generated web interface.

Together they provide everything needed for modern backend development.
...
- Enable GraphQL playgrounds
- Configure GraphQL extensions
- Create an Admin Dashboard
- Register models
- Customize the admin interface
- Combine GraphQL with the Admin Dashboard
...
If you are new to Flaxon, complete these lessons first:
...
The client describes exactly what data it wants.
...
Every GraphQL application consists of several parts.

| Component | Purpose |
|------------|----------|
| Schema | Defines the API |
| Query | Reads data |
| Mutation | Creates or updates data |
| Subscription | Real-time updates |
| Resolver | Returns data |
| Type | Defines objects |
| Scalar | Primitive values |
| Directive | Changes query behavior |
...
Flaxon includes GraphQL support out of the box.
...
Every GraphQL application starts with a schema.
...
Usually you provide a root query.
...
```python
from flaxon.graphql import ObjectType
from flaxon.graphql import Field
from flaxon.graphql import String
...
The GraphQL endpoint becomes available at
...
Arguments allow clients to customize queries.
...
Instead of embedding values directly inside queries, use variables.
...
Most APIs return structured data.
...
GraphQL excels at nested relationships.
...
GraphQL can return collections.

```python
from flaxon.graphql import List
...
Flaxon provides several built-in scalar types.
...
After enabling GraphQL, Flaxon automatically provides:

| Endpoint | Purpose |
|-----------|----------|
| `/graphql` | GraphQL API |
| `/graphql/graphiql` | GraphiQL IDE |
| `/graphql/altair` | Altair Playground |

These playgrounds make it easy to explore and test your schema during development.
...
By combining **Flaxon GraphQL** with the **Flaxon Admin Dashboard**, you can build complete backend applications using a single framework.

GraphQL gives clients flexible access to your data through queries, mutations, and subscriptions, while the Admin Dashboard provides an automatic interface for managing that same data. Since both layers share models, validation, services, authentication, and permissions, your application stays clean, consistent, and easy to maintain.

This architecture works well for everything from small personal projects to enterprise systems such as e-commerce platforms, CMSs, SaaS products, hospital systems, school management software, inventory applications, and social networks.