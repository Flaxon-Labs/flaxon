...
```python
async def push(task: Task) -> None
Push a task to the queue.

pop
python
async def pop(timeout: float | None = None) -> Task | None
...
schedule
python
def schedule(task: Task, delay: int | None = None, at: datetime | None = None, interval: int | None = None) -> None
Schedule a task.
